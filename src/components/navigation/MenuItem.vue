...
<template>
	<li v-if="hasDropdown" class="menu-item" :class="{ active: isActive }">
		...
		<ul class="menu-item__dropdown dropdown" :class="{ visible: isOpen }">
			<li v-for="item in dropdown" :key="item">
				<RouterLink :to="{ name: `${item}` }" class="dropdown__link" :aria-label="`Go to ${item} page`"
	...
	<li v-else class="menu-item">
		<RouterLink :to="{ name: name }" class="menu-item__link"
			:class="{ 'active': name === 'projects' && route.name === 'project' }" @click="chooseItem">
			<span class="menu-item__link-text">
				{{ t(`common.${name}`) }}
			</span>
		</RouterLink>
